...
# Module imports
from plane.db.models import Issue, User, ProjectMember
...
def analytic_export_task(email, data, slug):
    ...
    try:
        ...
        current_role_request.set(SimpleNamespace(user=user, rbac_capability="analytics.export"))
        allowed_projects = scoped_queryset(
        ...
        if requested:
            allowed_projects = allowed_projects.filter(project_id__in=requested)
        data = {**data, "project": [str(project_id) for project_id in allowed_projects]}
        if not data["project"]:
            return
        filters = issue_filters(data, "POST")
        queryset = scoped_queryset(Issue.issue_objects.all()).filter(**filters, workspace__slug=slug)
